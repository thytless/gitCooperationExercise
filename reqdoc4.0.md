...
或点击右下的小图标，使用已有账户绑定的第三方账户扫描二维码登录。关于绑定第三方账户见<a href="#h3219">3.2.1.9节</a>
...
在首页登录界面选择找回密码，输入绑定的邮箱/手机，系统检查数据库中绑定信息的存在，不存在返回错误，存在则将发送验证码。用户接收验证码并输入，核对无误后，告知用户原用户名，并请求输入新密码、确认密码，点击提交后账户找回完毕。
