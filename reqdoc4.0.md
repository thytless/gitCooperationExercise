...
点击成功案例操作按钮，会有添加和修改两个选项。点击添加，将会显示已经完成的测试案例，点击某一测试案例将会显示详细信息：申请人/单位，测试开始时间，结束时间，测试结果，测试费用等。同时带有展示和取消两个按钮，点击取消按钮将会回到已完成测试案例的页面。点击展示按钮将会显示目前网站内提供的所有分类（具体内容上文有讲），点击某一分类即为申请将这个成功案例展示到这个分类之中。点击修改，机会显示目前为止所有正在展示的成功案例，点击某一测试案例将会显示详细信息：申请人/单位，测试开始时间，结束时间，测试结果，测试费用等。同时会有删除和修改两个按钮，点击删除即可删除该案例，点击修改即可对这个案例进行修改。所有操作将会提交给管理员审核。



##### 3.2.2.16 审核案例

点击案例审核按钮，将会显示目前为止所有的案例操作申请（修改/添加/删除），点击某一申请将会显示详细信息：申请人、案例内容等，带有一个文本框以及（通过/驳回）2个按钮。点击通过即可通过审核，将该请求提交给更上一层的人员审核，如果已经是最高的审核人员，操作即通过审核。如果要点击驳回，必须在文本框里给出驳回的理由，驳回后的请求将会退回给申请人，并告知修申请人驳回的理由。 当没有账户管理员时，注册申请会提交给超级管理员。存在账户管理员时则不会。 管理员注册通过后会得到该类型管理员的默认权限。
